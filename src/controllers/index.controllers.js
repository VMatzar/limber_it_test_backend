...
const deleteCourse = async (req, res) => {
    ...
    await pool.query('DELETE FROM COURSES WHERE COURSE_ID = $1', [id], (error, results) => {
        if (error) {
            res.status(500).json({
                message: "Existen estudiante(s) asignados a este curso, primero deben desasignarse para poder eliminar este curso"
            });
        } else {
...
const deleteRegistration = async (req, res) => {
    ...
    await pool.query('DELETE FROM REGISTRATION WHERE STUDENT_ID = $1 AND COURSE_ID = $2',
        [
            ...
            if (error) throw error;
            res.json(`registration data deleted succesfully`);
        });
};
...
const postRegistration = async (req, res) => {
    ...
    await pool.query('INSERT INTO REGISTRATION (STUDENT_ID, COURSE_ID) VALUES ($1, $2)',
        ...
        (error, results) => {
            if (error) throw error;
            res.status(201).json({
                message: "Registration data added Succesfully",
                body: {
...
module.exports = {
    ...
    getCourses, getStudentCourses, getCourseById, updateCourse, deleteCourse, postCourse,
    deleteRegistration, postRegistration
}